...
**GPU Version**
```bash
docker pull berlius/kulitta:gpu
```

**CPU Version**
```bash
docker pull berlius/kulitta:cpu
```
...
```
git clone https://github.com/berlius/DeepLearningDockerfile/Kulitta
cd Kulitta

sudo docker build -t berlius/kulitta:gpu -f Dockerfile.gpu .
```
### CPU version
...
|`berlius/kulitta:gpu`   | This the image that you want to run. The format is `image:tag`. In our case, we use the image `kulitta` and tag `gpu` |
