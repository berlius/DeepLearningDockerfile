...
**GPU Version**
```bash
xhost + ; nvidia-docker run -it -p 8888:8888 -p 6006:6006 -v /sharedfolder:/root/sharedfolder -e DISPLAY=unix$DISPLAY berlius/microsoft-malmo:gpu bash
```
...
|`berlius/microsoft-malmo:gpu`   | This the image that you want to run. The format is `image:tag`. In our case, we use the image `dl-docker` and tag `gpu` |
