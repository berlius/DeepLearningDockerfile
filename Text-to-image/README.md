## Text-to-image Docker image for Deep Learning

Here are Dockerfile to get you up and running with a fully functional [text-to-image](https://github.com/paarthneekhara/text-to-image) installation. 
...
**GPU Version**
```bash
docker pull berlius/text-to-image:gpu
```

**CPU Version**
```bash
docker pull berlius/text-to-image:cpu
```
...
```
git clone https://github.com/berlius/DeepLearningDockerfile/Text-to-image
cd Text-to-image

sudo docker build -t berlius/text-to-image:gpu -f Dockerfile.gpu .
```
### CPU version
...
xhost + ; sudo nvidia-docker run -it -p 8888:8888 -p 6006:6006 -p 8000:8000 -v `pwd`:/root/sharedfolder -v /mnt/sda4:/root/sda4 --privileged --device=/dev/snd:/dev/snd -v /tmp/.X11-unix:/tmp/.X11-unix:ro -e DISPLAY=unix$DISPLAY berlius/text-to-image:gpu lxterminal
```
...
xhost + ; sudo nvidia-docker run -it -p 8888:8888 -p 6006:6006 -p 8000:8000 -v `pwd`:/root/sharedfolder -v /mnt/sda4:/root/sda4 --privileged --device=/dev/snd:/dev/snd -v /tmp/.X11-unix:/tmp/.X11-unix:ro -e DISPLAY=unix$DISPLAY berlius/text-to-image:cpu lxterminal
```
In "lxterminal" only for the first time installation:
...
|`berlius/text-to-image:gpu`   | This the image that you want to run. The format is `image:tag`. In our case, we use the image `kulitta` and tag `gpu` |
